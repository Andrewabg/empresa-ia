...
import { getPecaComVersoes, toPecaView } from '@/data/pecas'
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!r.ok) return NextResponse.json({ ok: false, error: r.output }, { status: 422 })
    const peca = await getPecaComVersoes(id)
    if (!peca) return NextResponse.json({ ok: false, error: r.output }, { status: 422 })
    return NextResponse.json({ ok: true, peca: toPecaView(peca), aviso: r.output })
  } catch (err) {
    console.error('[POST /api/estudio/pecas/:id/restaurar]', err)
