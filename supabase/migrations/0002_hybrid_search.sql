...
with full_text as (
  select note_id, chunk_index, content,
    row_number() over (order by ts_rank_cd(fts, websearch_to_tsquery('simple', query_text)) desc) as rank_ix
  from note_chunks
...
order by score desc
-- cap em 30 para limitar custo; ajuste se precisar de mais resultados
